...
def eff_vol(avg_rets, cov_mat, target):
    ...
    return sco.minimize(
        ...
        bounds=((0.0, 1.0),) * num_assets,
        constraints=(
            {"type": "eq", "fun": lambda x: pret(x) - target},
...
def eff_ret(avg_rets, cov_mat, target):
    ...
    return sco.minimize(
        ...
        bounds=((0.0, 1.0),) * num_assets,
        constraints=(
            {"type": "eq", "fun": lambda x: pvol(x) - target},
            {"type": "eq", "fun": lambda x: np.sum(x) - 1},
        ),
    )
...
def display_ef(avg_rets, cov_mat, rfr, bm_vol):
    ...
    print(f"M2: {max_sharpe_m2:.2f}")
    print(f"Sharpe ratio: {max_sharpe_sharpe:.2f}")
    print(f"Annualised Return: {max_sharpe_ret:.2f}")
    print(f"Annualised Volatility: {max_sharpe_vol:.2f}\n")
    display(allocations(max_sharpe_val, colnames))

    print("-" * 80)
    print("Minimum Volatility Portfolio Allocation\n")
    print(f"M2: {min_vol_m2:.2f}")
    print(f"Sharpe ratio: {min_vol_sharpe:.2f}")
    print(f"Annualised Return: {min_vol_ret:.2f}")
    print(f"Annualised Volatility: {min_vol_vol:.2f}\n")
